...
ind = sys.argv[1]
ind = round(float(ind))
hbpostdenom = sys.argv[2]
hbpostdenom = float(hbpostdenom)
hBpostdenom = sys.argv[3]
hBpostdenom = float(hBpostdenom)
xpre = sys.argv[4]

x = round(float(xpre),5)
...
xpost_range = np.arange(0,1.1,0.1)

hbpost, hBpost = (1-rpost/spost)/hbpostdenom,(rpost/spost)/hBpostdenom
...
output_writer = pd.ExcelWriter('Haploid_ER'+str(ind)+'_'+str(xpre)+'.xlsx')
...
for xpost in xpost_range: 
    ...
    mApre = 0 #np.random.random()*min(y/(1-y),1) if y != 1 else np.random.random()
    tApre = 1 #(1-y)*mApre/y if y != 0 else np.random.random()
...
ExtDF = pd.DataFrame(ExtProb, columns = xpost_range, index = [x])
